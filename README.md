...
### 選擇性：進階環境變數設定
如果您需要連線到自訂的小智伺服器，或是在特殊網路環境下，可以使用以下環境變數 (在 Settings > Environment Variables 新增)：

| 變數名稱 | 說明 | 預設值 (Default) | 範例 (Example) |
| :--- | :--- | :--- | :--- |
| `WS_URL` | 小智 WebSocket 伺服器地址 | `wss://api.tenclass.net/xiaozhi/v1/` | `wss://your-custom-server.com/ws/` |
| `OTA_VERSION_URL` | OTA 版本檢查地址 | `https://api.tenclass.net/xiaozhi/ota/` | `https://your-custom-server.com/ota/` |
| `WS_PROXY_URL` | 本地代理地址 (覆寫預設值) | (自動偵測 Port 8080) | `wss://your-backend.zeabur.app` |

> **注意**：設定 `WS_PROXY_URL` 後，前端「設定」頁面中的「本地代理地址」將會優先採用此值，而不是自動生成的 `:{PORT}`。這在某些轉發或代理環境下很有用。
*   **Networking (網路)** > **Domain**:
